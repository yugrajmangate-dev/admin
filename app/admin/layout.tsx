...
function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  // Show loading spinner while role is being resolved
  if (!role) {
    return (
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-slate-900 font-sans flex flex-col md:flex-row">
      ...
      {/* Mobile Header */}
      <div className="md:hidden flex items-center justify-between bg-white border-b border-gray-200 px-4 py-3 z-20 sticky top-0">
      ...
      <aside className={`
        ...
        md:flex flex-col fixed md:sticky top-0 left-0 h-screen w-64 bg-white border-r border-gray-200 z-30
        shadow-sm
      `}>
        {/* Logo */}
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all
                  ${isActive 
                    ? "bg-[#FF4F5A]/8 text-[#FF4F5A] font-semibold shadow-sm" 
        ...
        <div className="p-4 border-t border-gray-100 space-y-1">
          <div className="flex items-center gap-3 px-4 py-3 mb-1">
            <div className="h-8 w-8 rounded-full bg-gradient-to-br from-[#FF4F5A] to-orange-400 flex items-center justify-center text-white text-xs font-bold shrink-0">
              {avatarInitials}
            </div>
          ...
          <Link href="/admin/settings" className="flex w-full items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-slate-500 hover:bg-gray-50 hover:text-slate-900 transition-all">
            <Settings className="h-4 w-4" />
            Settings
          </Link>
          <button 
            onClick={handleLogout}
            className="flex w-full items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-red-500 hover:bg-red-50 transition-all"
          >
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-y-auto w-full">
        {/* Top Navbar */}
        <header className="hidden md:flex sticky top-0 z-20 w-full h-16 bg-white/90 backdrop-blur-md border-b border-gray-200 items-center justify-between px-8 shadow-sm">
          <h1 className="text-lg font-semibold text-slate-800 capitalize">
             {pathname?.split("/").filter(Boolean).slice(-1)[0]?.replace(/-/g, " ") || "Dashboard"}
          </h1>
          <div className="flex items-center gap-3">
            {isSuper && (
              <span className="flex items-center gap-1.5 text-xs font-bold text-[#FF4F5A] bg-[#FF4F5A]/5 px-3 py-1.5 rounded-full border border-[#FF4F5A]/10">
        ...
        <main className="p-4 sm:p-8 flex-1 w-full max-w-[1400px] mx-auto">
          {children}
        </main>
      </div>
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Standalone pages — no sidebar/navbar needed
